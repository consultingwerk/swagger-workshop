...
&SCOPED-DEFINE VERSION 2.0              /* decimal version number */

&SCOPED-DEFINE WEBSTREAM
...
DEFINE INPUT        PARAMETER  p_include-file  AS CHARACTER NO-UNDO.

/* Supported options.  Each preprocessor evaluates to TRUE or FALSE
   depending if the option is set or not. */
&SCOPED-DEFINE DEBUG-IS-SET CAN-DO(p_options,"debug":U)
&SCOPED-DEFINE WEB-OBJECT-IS-SET CAN-DO(p_options,"web-object":U)
&SCOPED-DEFINE INCLUDE-IS-SET CAN-DO(p_options,"include":U)
&SCOPED-DEFINE GET-OPTIONS-IS-SET CAN-DO(p_options,"get-options":U)
&SCOPED-DEFINE KEEP-META-CONTENT-TYPE CAN-DO(p_options,"keep-meta-content-type":U)
...
&SCOPED-DEFINE INITIAL-STATE    0
&SCOPED-DEFINE NORMAL-STATE     1
&SCOPED-DEFINE STMNT-TAG-OPEN   2
&SCOPED-DEFINE STMNT-4GL        4
&SCOPED-DEFINE EXPR-TAG-OPEN    8
&SCOPED-DEFINE EXPR-4GL         16
...
{&_proparse_ prolint-nowarn(worktablekeyword)}
DEFINE WORK-TABLE tag NO-UNDO
  FIELD tag-state   AS INTEGER   FORMAT ">>9":U INITIAL {&STMNT-TAG-OPEN}
  FIELD open-tag    AS CHARACTER FORMAT "x(10)":U
  FIELD close-tag   AS CHARACTER FORMAT "x(10)":U
  .
...
IF html-file = ? THEN DO: /* ERROR: cannot find HTML file */
  UNDO, THROW NEW Progress.Lang.AppError (
        SUBSTITUTE("NOTFOUND, input file &1 was not found":U, p_html-file), 0).
END.
...
DO WHILE TRUE
  ...
  IF meta-start > 0 THEN DO:
    ...
    IF meta-end = ? THEN DO:
      ...
      ASSIGN in-buf = SUBSTRING(in-buf, meta-start, -1, "CHARACTER":U) + " ":U +
                      TRIM(tmp-buf).
    END.
    ELSE
  ...
  ELSE DO:
    ...
    IF TRIM(in-buf) BEGINS "ROUTINE-LEVEL ON":U OR
       TRIM(in-buf) BEGINS "BLOCK-LEVEL ON":U OR
       TRIM(in-buf) BEGINS "USING ":U THEN
      cTopStatements = cTopStatements +
                       (IF (cTopStatements GT "":U) EQ TRUE THEN
                          CHR(1)
                        ELSE "":U) +
                       in-buf.
    ASSIGN
  ...
  IF meta-start > 0 THEN DO:
    ...
    ASSIGN tmp-buf = SUBSTRING(in-buf, meta-start, meta-end - meta-start +
                               LENGTH(html-close-tag, "CHARACTER":U), "CHARACTER":U).
    IF debug-mode THEN
      MESSAGE "meta tag:":U tmp-buf.
    ...
    IF meta-name <> "" THEN DO:
      ...
      IF attr-start > 0 THEN DO:
        ASSIGN
          attr-start = attr-start + LENGTH(attr-name, "CHARACTER":U) + 1
          /* Extract after CONTENT attribute removing leading spaces
             and equals sign. */
          tmp-buf = LEFT-TRIM(SUBSTRING(tmp-buf, attr-start, -1, "CHARACTER":U), " =":U)
          attr-start = 1
          /* See if the first character is a quote */
          qchar = SUBSTRING(tmp-buf, 1, 1, "CHARACTER":U) .
        IF debug-mode THEN
          MESSAGE "CONTENT attr:":U tmp-buf "quote:":U qchar.
        ...
        IF qchar = '"':U OR qchar = "'":U THEN DO:
          ...
          IF debug-mode THEN
            MESSAGE "attr-start:":U attr-start "attr-end:":U attr-end.
          ASSIGN meta-value =
            (IF attr-end > 0 THEN
                SUBSTRING(tmp-buf, attr-start, attr-end - attr-start, "CHARACTER":U)
             ELSE "").
        END.
        ...
        ELSE DO:
          ...
          IF attr-end = 0 THEN
            ...
            attr-end = LENGTH(RIGHT-TRIM(tmp-buf," ":U), "CHARACTER":U) -
                       LENGTH(html-close-tag, "CHARACTER":U).
          IF debug-mode THEN
            MESSAGE "attr-start:":U attr-start "attr-end:":U attr-end.
          ASSIGN meta-value =
            (IF attr-end > 0 THEN
                SUBSTRING(tmp-buf, attr-start, attr-end - attr-start + 1, "CHARACTER":U)
             ELSE "").
        END.
        IF debug-mode THEN
          MESSAGE "meta-name:":U meta-name "meta-value:":U meta-value.
...
IF NOT {&WEB-OBJECT-IS-SET} AND
  ...
  ASSIGN p_options = p_options +
    ...
    "web-object":U.
  IF debug-mode THEN
    MESSAGE "no generation type specified, setting to web-object":U.
END.
...
IF p_output-file = "" THEN DO:
  ...
  IF i > 0 THEN
    p_output-file = SUBSTRING(p_html-file,1,i, "CHARACTER":U) +
  ...
  CASE curr-state:
    ...
    */
    WHEN {&INITIAL-STATE} THEN DO:
      ...
      IF line-num > 0 THEN DO:
        IF NOT (TRIM(out-buf) BEGINS "ROUTINE-LEVEL ON":U) AND
           NOT (TRIM(out-buf) BEGINS "BLOCK-LEVEL ON":U) AND
           NOT (TRIM(out-buf) BEGINS "USING ":U) THEN
    ...
    */
    WHEN {&NORMAL-STATE} THEN DO:
      ...
      FOR EACH tag ON ERROR UNDO, THROW:
        ASSIGN tag-start = INDEX(in-buf, tag.open-tag, start-offset).
        IF debug-mode AND tag-start > 0 THEN
            MESSAGE "tag":U tag.open-tag "found on line":U line-num
                "at offset":U tag-start
                VIEW-AS ALERT-BOX.
        ...
        IF tag-start > 0 AND (tag-start-save = 0 OR
          ...
          IF debug-mode THEN
            MESSAGE "saved tag":U VIEW-AS ALERT-BOX.
        END.
      END.
    ...
    WHEN {&EXPR-4GL} THEN DO:
      ...
      IF tag-start > 0 THEN DO:
        ASSIGN
          ...
          out-buf = out-buf + RIGHT-TRIM(SUBSTRING(in-buf, start-offset,
                               tag-start - start-offset, "CHARACTER":U)) +
      ...
      ASSIGN
          ...
          out-buf = out-buf + SUBSTRING(in-buf, start-offset,
                              -1, "CHARACTER":U).
      ASSIGN curr-state = {&INITIAL-STATE}.  /* read next line */
    END.
